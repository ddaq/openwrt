...
if [ "$RPC_AUTH" == "true" ]; then
	RPC_USERNAME=$(uci get transmission.@transmission[0].rpc_username)
	RPC_PASSWORD=$(uci get transmission.@transmission[0].rpc_password)
fi
...
echo $RPC_USERNAME $RPC_PASSWORD
PARAMETER="$1"
[ "$PARAMETER" = "." ] && PARAMETER=" "

if [ "$RPC_AUTH" == "true" ]; then
	TORRENTS=$($TRANSMISSION_REMOTE -n $RPC_USERNAME:$RPC_PASSWORD -l 2>/dev/null)
else
...
for TORRENT in $PARAMETER; do
	...
	if [ "$RPC_AUTH" == "true" ]; then
		$TRANSMISSION_REMOTE -n $RPC_USERNAME:$RPC_PASSWORD -t $TORRENT -i | sed -nr 's/ *Name: ?(.*)/\1/p'
	else
	...
	do
		if [ ! -z "$TRACKER" ]; then
			...
			if [ "$RPC_AUTH" == "true" ]; then
				$TRANSMISSION_REMOTE -n $RPC_USERNAME:$RPC_PASSWORD -t $TORRENT -td $TRACKER 1>/dev/null 2>&1 
			else
