...
a.datatype = "ipaddr"
a.optional = false
sys.net.ipv4_hints(function(mac, name) a:value(mac, "%s (%s)" %{ mac, name }) end)

a = s:option(Value, "macaddr", translate("MAC Address"))
a.datatype = "macaddr"
a.optional = false
sys.net.mac_hints(function(ip, name) a:value(ip, "%s (%s)" %{ ip, name }) end)

a = s:option(ListValue, "ifname", translate("Interface"))
for _, iface in ipairs(ifaces) do
	--if iface ~= "lo" and not iface:match("^wdev.*") then 
	if iface ~= "lo" then 
		a:value(iface) 
	end
end
