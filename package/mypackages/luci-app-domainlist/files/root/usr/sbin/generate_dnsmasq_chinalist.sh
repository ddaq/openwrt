...
_process(){
    ...
    _TMP_CHINA_LIST_FILE=$_WORKING_DIR/accelerated-domains.china.conf
    ...
    curl -s -L $_CURL_EXTARG -o$_TMP_CHINA_LIST_FILE $_BASE_URL
    if [ $? != 0 ]; then
    ...
    if [ ! -z $EXCLUDE_DOMAIN_FILE ]; then
        ...
        do
            cat $_TMP_CHINA_LIST_FILE | grep -vF -f $EXCLUDE_DOMAIN_FILE > $_CHINA_LIST_FILE
        done
        printf 'Domains in exclude domain file '$EXCLUDE_DOMAIN_FILE'... ' && _green 'Deleted\n'
    else
        cat $_TMP_CHINA_LIST_FILE > $_CHINA_LIST_FILE
    fi

    # Add extra domains
    if [ ! -z $EXTRA_DOMAIN_FILE ]; then
        cat $EXTRA_DOMAIN_FILE >> $_CHINA_LIST_FILE
